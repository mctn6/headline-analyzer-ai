...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_GOOGLE_API_KEY) {
    return new NextResponse(JSON.stringify({ error: "API key is not set" }), { status: 500 });
  }
  ...
  try {
    ...
    try {
      const analysisResult = JSON.parse(text.replace(/`/g, "").replace("json", ""));
      return new NextResponse(JSON.stringify(analysisResult), { status: 200 });
    } catch (jsonError) {
      console.error("Error parsing analysis result:", jsonError);
      return new NextResponse(JSON.stringify({ error: "Error parsing analysis result. Please try again." }), { status: 500 });
    }
  } catch (error) {
    console.error("Error analyzing text:", error);
    return new NextResponse(JSON.stringify({ error: "Error analyzing text. Please try again." }), { status: 500 });
  }
}
